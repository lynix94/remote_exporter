...
class RedisPrometheusExporter:
    ...
    def _init_metrics(self):
        ...
        # Server info
        self.redis_up = self.metric_factory.gauge('redis_up', 'Redis instance is up', ['address'])
        self.redis_info = self.metric_factory.info('redis_info', 'Redis server info', ['address'])
        
        # Memory metrics
        self.redis_memory_used_bytes = self.metric_factory.gauge('redis_memory_used_bytes', 'Used memory in bytes', ['address'])
        self.redis_memory_max_bytes = self.metric_factory.gauge('redis_memory_max_bytes', 'Max memory in bytes', ['address'])
        self.redis_memory_rss_bytes = self.metric_factory.gauge('redis_memory_rss_bytes', 'RSS memory in bytes', ['address'])
        self.redis_memory_fragmentation_ratio = self.metric_factory.gauge('redis_memory_fragmentation_ratio', 'Memory fragmentation ratio', ['address'])
        
        # Client metrics
        self.redis_connected_clients = self.metric_factory.gauge('redis_connected_clients', 'Connected clients', ['address'])
        self.redis_blocked_clients = self.metric_factory.gauge('redis_blocked_clients', 'Blocked clients', ['address'])
        
        # Stats metrics
        self.redis_commands_processed_total = self.metric_factory.counter('redis_commands_processed_total', 'Total commands processed', ['address'])
        self.redis_connections_received_total = self.metric_factory.counter('redis_connections_received_total', 'Total connections received', ['address'])
        self.redis_keyspace_hits_total = self.metric_factory.counter('redis_keyspace_hits_total', 'Keyspace hits', ['address'])
        self.redis_keyspace_misses_total = self.metric_factory.counter('redis_keyspace_misses_total', 'Keyspace misses', ['address'])
        self.redis_evicted_keys_total = self.metric_factory.counter('redis_evicted_keys_total', 'Evicted keys', ['address'])
        self.redis_expired_keys_total = self.metric_factory.counter('redis_expired_keys_total', 'Expired keys', ['address'])
        
        # Persistence metrics
        self.redis_rdb_last_save_time = self.metric_factory.gauge('redis_rdb_last_save_time', 'Last RDB save timestamp', ['address'])
        self.redis_rdb_changes_since_last_save = self.metric_factory.gauge('redis_rdb_changes_since_last_save', 'Changes since last save', ['address'])
        
        # Replication metrics
        self.redis_connected_slaves = self.metric_factory.gauge('redis_connected_slaves', 'Connected slaves', ['address'])
        self.redis_master_repl_offset = self.metric_factory.gauge('redis_master_repl_offset', 'Master replication offset', ['address'])
        
        # Cluster metrics
        self.redis_cluster_enabled = self.metric_factory.gauge('redis_cluster_enabled', 'Cluster enabled', ['address'])
        self.redis_cluster_state = self.metric_factory.gauge('redis_cluster_state', 'Cluster state (1=ok, 0=fail)', ['address'])
        
        # Database metrics
        self.redis_db_keys = self.metric_factory.gauge('redis_db_keys', 'Keys in database', ['address', 'db'])
        self.redis_db_expires = self.metric_factory.gauge('redis_db_expires', 'Keys with expiration', ['address', 'db'])
    ...
    async def _collect_single_instance(self, addr: str, client: RedisClient):
        ...
        try:
            ...
            # Server is up
            self.redis_up.labels(address=addr).set(1)
            
            # Server info
            self.redis_info.labels(address=addr).info({
            ...
            if 'used_memory' in info:
                self.redis_memory_used_bytes.labels(address=addr).set(int(info['used_memory']))
            if 'maxmemory' in info:
                self.redis_memory_max_bytes.labels(address=addr).set(int(info['maxmemory']))
            if 'used_memory_rss' in info:
                self.redis_memory_rss_bytes.labels(address=addr).set(int(info['used_memory_rss']))
            if 'mem_fragmentation_ratio' in info:
                self.redis_memory_fragmentation_ratio.labels(address=addr).set(float(info['mem_fragmentation_ratio']))
            
            # Client metrics
            if 'connected_clients' in info:
                self.redis_connected_clients.labels(address=addr).set(int(info['connected_clients']))
            if 'blocked_clients' in info:
                self.redis_blocked_clients.labels(address=addr).set(int(info['blocked_clients']))
            
            # Stats metrics
            if 'total_commands_processed' in info:
                self.redis_commands_processed_total.labels(address=addr)._value._value = int(info['total_commands_processed'])
            if 'total_connections_received' in info:
                self.redis_connections_received_total.labels(address=addr)._value._value = int(info['total_connections_received'])
            if 'keyspace_hits' in info:
                self.redis_keyspace_hits_total.labels(address=addr)._value._value = int(info['keyspace_hits'])
            if 'keyspace_misses' in info:
                self.redis_keyspace_misses_total.labels(address=addr)._value._value = int(info['keyspace_misses'])
            if 'evicted_keys' in info:
                self.redis_evicted_keys_total.labels(address=addr)._value._value = int(info['evicted_keys'])
            if 'expired_keys' in info:
                self.redis_expired_keys_total.labels(address=addr)._value._value = int(info['expired_keys'])
            
            # Persistence metrics
            if 'rdb_last_save_time' in info:
                self.redis_rdb_last_save_time.labels(address=addr).set(int(info['rdb_last_save_time']))
            if 'rdb_changes_since_last_save' in info:
                self.redis_rdb_changes_since_last_save.labels(address=addr).set(int(info['rdb_changes_since_last_save']))
            
            # Replication metrics
            if 'connected_slaves' in info:
                self.redis_connected_slaves.labels(address=addr).set(int(info['connected_slaves']))
            if 'master_repl_offset' in info:
                self.redis_master_repl_offset.labels(address=addr).set(int(info['master_repl_offset']))
            
            # Cluster metrics
            if 'cluster_enabled' in info:
                self.redis_cluster_enabled.labels(address=addr).set(int(info['cluster_enabled']))
            ...
            for key, value in info.items():
                if key.startswith('db'):
                    ...
                    if 'keys' in db_info:
                        self.redis_db_keys.labels(address=addr, db=db_name).set(int(db_info['keys']))
                    if 'expires' in db_info:
                        self.redis_db_expires.labels(address=addr, db=db_name).set(int(db_info['expires']))
            
        except Exception as e:
            self.logger.error(f"Failed to collect from {addr}: {e}")
            self.redis_up.labels(address=addr).set(0)
...
def run_redis_exporter(args=None):
    ...
    parser.add_argument('--cluster-mode', action='store_true',
                       help='Enable Redis Cluster mode with automatic node discovery')
