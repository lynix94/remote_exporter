...
Examples:
  ...
  %(prog)s conf --config config.yaml
    ...
    redis_parser.add_argument('--redis-password',
                             help='Redis password')
    redis_parser.add_argument('--cluster-mode', action='store_true',
    ...
    if args.command == 'node':
        ...
        if args.cluster_mode:
            redis_args.append('--cluster-mode')
...
def run_from_config(config_file: str, cli_exporter_port: int = None, cli_collect_interval: float = None, 
    ...
    for exporter_type, exporter_config, idx in exporter_instances:
        thread = threading.Thread(
            ...
            args=(exporter_type, exporter_config, log_level, shared_registry, shared_node_collector, exporter_port, collect_interval, max_concurrent, region),
            daemon=True,
    ...
    start_http_server(exporter_port, registry=shared_registry)
    logger.info(f"Shared exporter HTTP server started on port {exporter_port}")
    ...
    try:
        if exporter_type == 'node':
            ...
            if 'cloud_name' in config:
                default_labels['cloud'] = config['cloud_name']
