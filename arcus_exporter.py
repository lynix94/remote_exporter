    ...
    async def _collect_arcus_metrics(self, client: MemcachedClient, instance: str):
        ...
        try:
            ...
            for operation, count in lop_commands.items():
                self.arcus_lop_commands_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in sop_commands.items():
                self.arcus_sop_commands_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in bop_commands.items():
                self.arcus_bop_commands_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in mop_commands.items():
                self.arcus_mop_commands_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in lop_hits.items():
                self.arcus_lop_hits_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in lop_misses.items():
                self.arcus_lop_misses_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in sop_hits.items():
                self.arcus_sop_hits_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in sop_misses.items():
                self.arcus_sop_misses_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in bop_hits.items():
                self.arcus_bop_hits_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in bop_misses.items():
                self.arcus_bop_misses_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in mop_hits.items():
                self.arcus_mop_hits_total.labels(operation=operation, address=instance).set(count)
            ...
            for operation, count in mop_misses.items():
                self.arcus_mop_misses_total.labels(operation=operation, address=instance).set(count)
            
            # Update additional Arcus metrics
            self.arcus_heartbeat_count.labels(address=instance).set(stats.get('hb_count', 0))
            self.arcus_heartbeat_latency.labels(address=instance).set(stats.get('hb_latency', 0))
            self.arcus_prefix_count.labels(address=instance).set(stats.get('curr_prefixes', 0))
            self.arcus_sticky_items.labels(address=instance).set(stats.get('sticky_items', 0))
            self.arcus_sticky_bytes.labels(address=instance).set(stats.get('sticky_bytes', 0))
            self.arcus_outofmemorys.labels(address=instance).set(stats.get('outofmemorys', 0))
            
            # Additional metrics for collection oks
            if 'lop_create_oks' in stats:
                self.arcus_lop_commands_total.labels(operation='create_ok', address=instance).set(stats['lop_create_oks'])
            if 'sop_create_oks' in stats:
                self.arcus_sop_commands_total.labels(operation='create_ok', address=instance).set(stats['sop_create_oks'])
            if 'bop_create_oks' in stats:
                self.arcus_bop_commands_total.labels(operation='create_ok', address=instance).set(stats['bop_create_oks'])
            if 'mop_create_oks' in stats:
                self.arcus_mop_commands_total.labels(operation='create_ok', address=instance).set(stats['mop_create_oks'])
            
            if 'bop_mget_oks' in stats:
                self.arcus_bop_commands_total.labels(operation='mget_ok', address=instance).set(stats['bop_mget_oks'])
            if 'bop_smget_oks' in stats:
                self.arcus_bop_commands_total.labels(operation='smget_ok', address=instance).set(stats['bop_smget_oks'])
...
def run_arcus_exporter(args=None):
    ...
    try:
        ...
        if args.cloud_name:
            default_labels['cloud'] = args.cloud_name
